/*
  org.iterx.miru.dispatcher.resolver.resource.TestUriResourceResolver
...
import org.iterx.miru.dispatcher.resolver.resource.UriResourceResolver;

public class TestUriResourceResolver extends TestCase {

    private static final String PATH = 
        "org/iterx/miru/dispatcher/resolver/resource/TestUriResourceResolver.class";

    private URI absoluteUri, relativeUri, base;
	...
	base = new URI(absolute.substring
		       (0, absolute.length() - PATH.length()));
    }
	
    protected void tearDown() {

	absoluteUri = relativeUri = base = null;
    }
	...
	resourceResolver = new UriResourceResolver(base);
	assertNotNull(resourceResolver);
	assertEquals(base, resourceResolver.getBase());
	...
	assertEquals(base,
		     resourceResolver.getBase());
	try {
	    resourceResolver.setBase(null);
	    fail("Base uri set to null");
	}
	catch(IllegalArgumentException e) {}

	try {
	    resourceResolver.setBase(relativeUri);
